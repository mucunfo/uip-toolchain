...
- `uip login status` / `uip login --interactive` / `uip login tenant list` /
  `uip login tenant set RPA_Desenvolvimento`.
- CCS package validation against the authenticated Orchestrator Libraries feed with
  `uip resource libraries versions <CCS_*>` after the DEV tenant is selected. The local
  `.nupkgs` source used by offline `D-1q-CCS-AUTO` is suppressed in the
  publish review; publish has no local package fallback.
- Mandatory batch preflight inside `ccs-uip-publish`: after selection, the
  command authenticates, selects DEV, and runs the authenticated pre-publish
  review across every selected project before confirmation, version changes,
  packing, uploading, or downloading artifacts.
- `python -m uip_engine.cli audit-publish-handoff <bump> <source-root>
  <handoff-folder>` to compare externally generated handoff packages against
  the selected source projects and expected bumped versions before accepting
  the artifact set. It also compares the packaged `lib/<tfm>/project.json`
  descriptor against the source `project.json` after the expected version bump,
  with .NET type-name normalization, so a package with the right file
  name/version but stale dependencies, arguments, entrypoints, runtime options,
  or design options is rejected.
- `python -m uip_engine.cli audit-orchestrator-readiness <manifest.json>` as
  the read-only post-publish Orchestrator gate. The manifest supplies the
  concrete `packageId`, `packageVersion`, `processKey`, folder path/key and
  runtime type for each process; the command checks package visibility, process
  entry points/input schemas, process version binding, Package
  Requirements/resources, optional declared runtime resources
  (`requiredResources`) and runtime capacity.
  With `--resolve-config-assets`, it also reads the value of inferred
  `ArquivoConfiguracao_*` Text assets, opens the referenced workbook when it is
  accessible on the current machine, resolves `runtimeResourceHints`, and
  validates the resulting assets/credentials/queues/calendars.
- `python -m uip_engine.cli build-orchestrator-manifest <handoff-folder>
  --source-root <source-root> --out <manifest.json>` to generate that manifest
  read-only from the `.nupkg` set and the current Orchestrator process
  bindings, instead of hand-copying process keys. With `--source-root`, it also
  infers `ArquivoConfiguracao_*` bootstrap assets into `requiredResources` and
  records `runtimeResourceHints` for dynamic config keys that still need real
  values. It also copies required input argument names from `project.json` into
  `requiredInputArguments`. The manifest always copies package
  `targetFramework` and entry point from `lib/<tfm>/project.json`, so readiness
  can catch an Orchestrator process still configured for Windows-Legacy/net45 or
  a different XAML entry point.
- `python -m uip_engine.cli sync-orchestrator-processes <handoff-folder>
  --execute` when the existing Orchestrator processes still point to older
  package versions. Without `--execute`, it is a dry-run. With `--execute`, it
  updates process versions via `uip or processes update-version` and verifies
  with `uip or processes get`.
- `python -m uip_engine.cli orchestrator-smoke <manifest.json> --execute` for
  the controlled runtime proof. Without `--execute`, the command is dry-run and
  returns WARN. With `--execute`, it runs readiness first, starts one job per
  manifest entry with `uip or jobs start --wait-for-completion`, requires final
  state `Successful`, and checks that the job has no Error-level robot logs.
...
- `UiRobot.exe pack <project.json> -o <out> -v <version>` using a Studio/Robot
  ...
  installs, or `PATH`.
- post-pack package sanitation that removes source-control/build artifacts such
  as `content/.git`, `.tmp`, `.local`, `.publish-dev-handoff`, and Python/test
  caches from the generated `.nupkg`.
- post-pack repair that copies `content/project.json` into missing
  `lib/<tfm>/project.json` descriptors, followed by full handoff audit before
  upload: ZIP, `.nuspec`, id/version consistency, target frameworks, runtime
  descriptors, `project.json`, runtime DLL, dependency pins, and forbidden
  package members. The packaged descriptor is also compared against the source
  `project.json` after the version bump before upload.
- `uip or packages upload` to DEV.
- `uip or packages download` from DEV for the handoff `.nupkg`, followed by the
  same audit, the same source descriptor comparison, and a SHA-256 comparison
...
10. Run `ccs-uip-publish <bump> <root> --all` on the machine/session that can
    see `RPA_Desenvolvimento`; the authenticated online preflight is mandatory
    and runs before the command asks for upload/download confirmation.
11. Use `python -m uip_engine.cli audit-publish-handoff <bump> <source-root>
    <handoff-folder>` when packages are produced on another machine/network;
    this checks exact package count, package id/version, duplicates,
    unexpected files, descriptor drift from the source project, and the full
    `.nupkg` runtime audit.
12. Use `python -m uip_engine.cli audit-nupkg <file-or-folder>` for offline
    package inspection when handoff packages are produced outside this machine.
13. After upload, run `python -m uip_engine.cli sync-orchestrator-processes
    <handoff-folder>` as a dry-run. If the plan is correct, run the same command
    with `--execute` to update releases/processes to the uploaded versions. The
    command is fail-closed on missing processes, ambiguous matches, update
    failure, or failed post-update verification.
14. Run `python -m uip_engine.cli build-orchestrator-manifest
    <handoff-folder> --source-root <source-root> --out <manifest.json>` in the
    session that can see the DEV tenant. This fails on missing process bindings,
    stale process versions or ambiguous package/version matches. When
    `--source-root` is provided, it also infers bootstrap config assets and
    reports dynamic config-key hints that must be completed with the real
    runtime resource names for full readiness coverage. It also records
    `requiredInputArguments` from `project.json` so the next gate can fail
    before execution if a required job input was not supplied. It records
    package `targetFramework` and entry point from the runtime descriptor for
    comparison against `uip or processes get`.
15. Run `python -m uip_engine.cli audit-orchestrator-readiness
    <manifest.json>` before manual play/trigger activation. This is still
    read-only and checks package visibility, process binding, Package
    entry points/input schemas through `uip or packages entry-points`,
    Requirements, process type/status when exposed, package target
    framework/entry point against the process, optional `requiredResources`,
    folder runtime counts, machines assigned to the folder and unattended
    sessions for the selected runtime.
    Use `requiredResources` for runtime-only assets/credentials/queues/buckets/calendars
    whose names come from the real configuration file instead of Package
    Requirements. The gate validates these through official
    `uip resource assets|queues|buckets|bucket-files list` calls,
    `uip or calendars list`, and exact name matching after the CLI filter.
    Add `--resolve-config-assets` when the DEV
    session can read the `ArquivoConfiguracao_*` asset value and the referenced
    workbook path is reachable from the current machine; this resolves
    `runtimeResourceHints` automatically from the workbook `Settings`, `Assets`
    and `Credentials` sheets. If `requiredInputArguments` is present, add matching
    `inputArguments` to the manifest; `inputFile` is accepted for smoke, but
    read-only readiness can only warn because it does not inspect that file.
    The final proof of Robot cache extraction, runtime
    identity and first-run behavior is a controlled `uip or jobs start
    <process-key> --wait-for-completion` smoke job in the same folder/account/
    machine context.
16. Prefer the built-in smoke wrapper for that proof:
    `python -m uip_engine.cli orchestrator-smoke <manifest.json> --execute
    --timeout <seconds>`. It fails on non-Successful jobs and Error-level logs.
...
Offline package audit file: `src/uip_engine/nupkg_audit.py`.
Post-publish process version sync file: `src/uip_engine/orchestrator_sync.py`.
Post-publish manifest builder file: `src/uip_engine/orchestrator_manifest.py`.
Post-publish Orchestrator readiness file:
`src/uip_engine/orchestrator_readiness.py`.
Controlled Orchestrator smoke file: `src/uip_engine/orchestrator_smoke.py`.
