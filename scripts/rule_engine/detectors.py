...
def detect_regex_count(rule: Rule, fc: FileContext, pc) -> list[Finding]:
    params = rule.detect.get("params", {})
    pattern = params.get("pattern")
    if not pattern:
        return []
    matches = list(re.finditer(pattern, fc.active_content))
    n = len(matches)
    min_n = params.get("min")
    max_n = params.get("max")
    ...
    line = _line_for(fc.active_content, matches[0].start()) if matches else 1
    return [Finding(
        rule_id=rule.id, severity=rule.severity, category=rule.category,
        file=str(fc.path), line=line,
        message=f"{rule.title} ({n} ocorrências)",
        fix_mechanical=(rule.fix or {}).get("mechanical"),
        fix_prose=(rule.fix or {}).get("prose"),
    )]
...
@register("xml_descendant_count")
def detect_xml_descendant_count(rule: Rule, fc: FileContext, pc) -> list[Finding]:
    params = rule.detect.get("params", {})
    ...
    pattern = rf"<{re.escape(element)}\b"
    matches = list(re.finditer(pattern, fc.active_content))
    n = len(matches)
    ...
    return [Finding(
        rule_id=rule.id, severity=rule.severity, category=rule.category,
        file=str(fc.path), line=line,
        message=f"{rule.title} ({n} ocorrências de {element})",
        fix_mechanical=(rule.fix or {}).get("mechanical"),
        fix_prose=(rule.fix or {}).get("prose"),
    )]


@register("structural_order")
def detect_structural_order(rule: Rule, fc: FileContext, pc) -> list[Finding]:
    params = rule.detect.get("params", {})
    parent_open = params.get("parent_open")
    must_first = params.get("must_be_first_child")
    if not parent_open or not must_first:
        return []

    findings: list[Finding] = []
    content = fc.active_content

    for m in re.finditer(rf"<{re.escape(must_first)}>", content):
        before = content[: m.start()]
        parent_match = None
        for pm in re.finditer(parent_open, before):
            tag_text = before[pm.start():pm.start()+50]
            if tag_text.startswith(f"<{must_first}"):
                continue
            parent_match = pm
        if parent_match is None:
            continue
        seq_tag_end = before.index(">", parent_match.start()) + 1
        between = content[seq_tag_end: m.start()]
        cleaned = re.sub(
        ...
        if cleaned:
            findings.append(Finding(
                rule_id=rule.id, severity=rule.severity, category=rule.category,
                file=str(fc.path), line=_line_for(content, m.start()),
                message=f"{rule.title}: <{must_first}> deve ser primeiro filho",
                fix_mechanical=(rule.fix or {}).get("mechanical"),
                fix_prose=(rule.fix or {}).get("prose"),
            ))
    return findings
...
@register("config_xlsx_sheets")
def detect_config_xlsx_sheets(rule: Rule, fc: FileContext, pc) -> list[Finding]:
    if pc is None or not _is_main_or_pj(fc):
        return []
    config_path = _find_config_xlsx(pc)
    if config_path is None:
        return []
    ...
    return [Finding(
        rule_id=rule.id, severity=rule.severity, category=rule.category,
        file=str(config_path), line=1,
        message=f"{rule.title}: sheets faltando {sorted(missing)}",
        fix_mechanical=(rule.fix or {}).get("mechanical"),
        fix_prose=(rule.fix or {}).get("prose"),
    )]


@register("config_xlsx_columns")
def detect_config_xlsx_columns(rule: Rule, fc: FileContext, pc) -> list[Finding]:
    if pc is None or not _is_main_or_pj(fc):
        return []
    config_path = _find_config_xlsx(pc)
    if config_path is None:
    ...
    if not first_row:
        return []
    cols = {str(c) for c in first_row if c}
    ...
    return [Finding(
        rule_id=rule.id, severity=rule.severity, category=rule.category,
        file=str(config_path), line=1,
        message=f"{rule.title}: sheet '{sheet}' faltam colunas {sorted(missing)}",
        fix_mechanical=(rule.fix or {}).get("mechanical"),
        fix_prose=(rule.fix or {}).get("prose"),
    )]


@register("config_xlsx_naming")
def detect_config_xlsx_naming(rule: Rule, fc: FileContext, pc) -> list[Finding]:
    if pc is None or not _is_main_or_pj(fc):
        return []
    config_path = _find_config_xlsx(pc)
    if config_path is None:
        return []
    params = rule.detect.get("params", {})
    pattern = params.get("pattern", r"^[A-Z][A-Za-z0-9_]+$")
    try:
        wb = openpyxl.load_workbook(config_path, read_only=True, data_only=True)
        bad: list[str] = []
        for sheet_name in wb.sheetnames:
    ...
    return [Finding(
        rule_id=rule.id, severity=rule.severity, category=rule.category,
        file=str(config_path), line=1,
        message=f"{rule.title}: chaves fora do padrão: {bad[:5]}",
        fix_mechanical=(rule.fix or {}).get("mechanical"),
        fix_prose=(rule.fix or {}).get("prose"),
    )]
